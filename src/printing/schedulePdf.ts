...
export function buildSchedulePdf(
  ...
  const text: PdfTextItem[] = [
    ...
    {
      text: `${rows.length} ligne(s) — ${now.toLocaleDateString("fr-FR")}`,
      xPt: widthPt - 145,
      yPt: top,
      sizePt: 7,
    },
  ...
  rows.forEach((row, index) => {
    ...
    const values = [
      ...
      row.reference,
      row.name,
      row.quantity.toLocaleString("fr-FR", { maximumFractionDigits: 2 }),
      row.unit,
    ];
    values.forEach((value, column) =>
      text.push({ text: String(value), xPt: columns[column] ?? margin, yPt: y, sizePt: fontSize }),
    );
  });
  ...
  return buildPdf(
    ...
    {
      title: `${projectName} — Nomenclature`,
      creator: "Plan Editor",
