...
export interface SpatialPointItem { pointM: PointM }
...
export class SpatialPointIndex<T extends SpatialPointItem> {
  ...
  private key(xM: number, yM: number) { return `${Math.floor(xM / this.cellSizeM)}:${Math.floor(yM / this.cellSizeM)}`; }
  query(point: PointM, radiusM: number): T[] {
    ...
    const result: T[] = [];
    for (let x = minX; x <= maxX; x += 1) for (let y = minY; y <= maxY; y += 1) result.push(...(this.buckets.get(`${x}:${y}`) ?? []));
    return result;
  }
}
