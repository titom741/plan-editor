...
import { computePrintRaster, previewDpiToFit } from "../../printing/sheetLayout";
...
export function SheetPreview({
  ...
  // Derived from the paper rather than fixed: at a constant DPI anything
  // above A4 was shown as a corner of itself, which is a print preview
  // answering its own question with a guess.
  const previewDpi = previewDpiToFit(layout);
  const raster = computePrintRaster(layout, previewDpi);
  const pxPerPt = previewDpi / 72;
  ...
  return (
    <div className="sheet-preview" aria-label="Aperçu de la feuille finale">
      <div className="sheet-preview__page" style={{ width: pageWidth, height: pageHeight }}>
        <div
          className="sheet-preview__frame"
          style={{
            left: frameLeft,
            top: frameTop,
            width: layout.frame.widthPt * pxPerPt,
            height: layout.frame.heightPt * pxPerPt,
          }}
        />
        <div
          className="sheet-preview__drawing"
          style={{
            left: drawingLeft,
            top: drawingTop,
        ...
        >
          <PrintCanvas
            ...
            showGrid={showGrid}
            labelDisplay={project.labelDisplay ?? DEFAULT_LABEL_DISPLAY}
            renderScale={previewDpi / 96}
            onReady={noop}
          />
        </div>
        <div
          className="sheet-preview__title"
          style={{
            left: frameLeft,
            bottom: layout.frame.yPt * pxPerPt,
            width: layout.frame.widthPt * pxPerPt,
            height: layout.titleBlock.heightPt * pxPerPt,
          }}
        >
          <strong>{project.name}</strong>
          <span>
            {sheet.name} · {formatScale(sheet.scaleDenominator)}
          </span>
          <small>{sheet.titleBlock?.client || project.location || " "}</small>
          <small>{sheet.titleBlock?.revision ? `Rév. ${sheet.titleBlock.revision}` : " "}</small>
        </div>
      </div>
    </div>
  );
