...
export function ScheduleDialog({ project, objects, onClose }: ScheduleDialogProps) {
  ...
  const downloadPdf = () => {
    ...
    downloadBlob(
      ...
      `${safeName(project.name)}-nomenclature.pdf`,
    );
  };
  return (
    <div className="dialog-backdrop" role="presentation" onMouseDown={onClose}>
      <section
        className="dialog schedule-dialog"
        role="dialog"
        aria-modal="true"
      ...
      >
        <div className="dialog__header">
          <h2 id="schedule-title">Nomenclature</h2>
          <button type="button" className="dialog__close" onClick={onClose} aria-label="Fermer">
            ✕
          </button>
        </div>
        {rows.length === 0 ? (
          <p>Le plan ne contient encore aucun élément.</p>
        ) : (
          <div className="schedule-dialog__table-wrap">
            <table>
              ...
              <tbody>
                {rows.map((row) => (
                  <tr key={`${row.layer}-${row.category}-${row.reference}-${row.name}`}>
                    <td>{row.layer}</td>
                    <td>{row.category}</td>
                    <td>{row.reference}</td>
                    <td>{row.name}</td>
                    <td>{row.quantity.toLocaleString("fr-FR", { maximumFractionDigits: 2 })}</td>
                    <td>{row.unit}</td>
                  </tr>
                ))}
              </tbody>
              <tfoot>
                <tr>
                  <th colSpan={6}>{rows.length} ligne(s) de nomenclature</th>
                </tr>
              </tfoot>
            </table>
          </div>
